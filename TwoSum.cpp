...
int* twoSum(int* nums, int numsSize, int target) {
    ...
    //借组一个中间数组
    bool *temp=(bool *)malloc((target+1)*sizeof(bool));
    if(temp==NULL){
        exit(EXIT_FAILURE);
        
    }
    memset(temp,false,(target+1)*sizeof(bool));
    int index=0; 
    int secondValue=0;
    while(index<numsSize){
    	if(nums[index]>target)
    		continue;
    	else if(!temp[nums[index]]){//如果 temp[nums[index]为false，则标记temp[target-nums[index]]为true
			 temp[target-nums[index]]=true; 
		}
		else{//找到第二个数的下标 
			result[1]=index+1;
	...
	if(index>=numsSize){//没有找到符合条件的 
		return  result;
	}
	else{
		//在nums中 寻找target-num[i]在nums中的下标即可
		for(int i=0;i<index;i++){
			if(nums[i]==target-secondValue){
				result[0]=i+1;
			}
		}
